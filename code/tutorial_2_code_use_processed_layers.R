...
#####  PART 1 ::: load pre-prepped environmental data  #####

### import processed environmental layers. These layers are already cropped and masked to the modeling area
...
occs <- humboldt::humboldt.occ.rarefy(in.pts = occs, colxy = 2:3, rarefy.dist = 10, rarefy.units = 'km', run.silent.rar = F)  # this line will throw an error. this is most likely due to incompatibilities between the humboldt package and the recent R and package updates. ignore this error and proceed
...
# We can see that our optimal model is built with LQHPT features and a regularization value of 1.0. This is the 4th model out of 12 models.
...
gplot(mess$mess) +
  ...
  scale_fill_gradientn(colors = rev(as.vector(ocean.thermal(22))),
                       ...
                       name = 'MESS',
                       breaks = c(minValue(mess$mess) + 900, maxValue(mess$mess) - 20),
                       labels = c('High', 'Low'),
                       trans = 'reverse') +
  xlab('Long') + ylab('Lat')
